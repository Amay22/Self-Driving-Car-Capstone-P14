...
class DBWNode(object):
    def __init__(self):
        ...
        # Initialize variables to store the data of subscribed topics below
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.velocity is not None and self.pose is not None and \
                self.twist is not None and self.waypoints is not None and delta_t > 0:
                current_speed = self.velocity.linear.x
                if self.dbw_enabled:
                    throttle, brake, steer = self.controller.control(self.waypoints, self.pose, current_speed,
                        self.twist.linear.x, self.twist.angular.z, delta_t)
                else:
                    self.controller.reset(current_speed)
            else:
